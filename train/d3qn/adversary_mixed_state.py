...
from .mixedneuralnet import D3QN_Mixed
...
class D3QN_Mixed_State_Opponent(BaseOpponent):
    """
    ...
    def __init__(self, env, state_mean, state_std, lines_attacked=[], 
                ...
                name=__name__, is_training=False, value_is_def=True,
        ...
        self.done = False
        self.def_frames2 = None
        self.kaist_frames2 = None
        ...
        # Load network graph
        self.policy_net = D3QN_Mixed(self.action_size,
    ...
    def _init_training(self):
        ...
        self.target_net = D3QN_Mixed(self.action_size,
                                self.observation_size,
                               self.action_space.dim_topo * 6,
                                #self.observation_size,
                                num_frames = self.num_frames)            
    ...
    def attack(self, observation, agent_action=None, env_action=None, budget=None, previous_fails=False):
        ...
        if len(self.def_frames) < self.num_frames:
            return self._do_nothing, 0
        ...
        if np.random.rand(1) < self.epsilon and self.is_training:
            ...
            a, _ = self.policy_net.predict_move(status, np.array(self.def_frames), np.array(self.kaist_frames))
            self.remaining_time = self.attack_duration
    ...
    def _reset_state(self, current_obs):
        ...
        self.def_state, self.kaist_state = self.convert_obs(self.obs)
        self.done = False
    ...
    def _save_current_frame(self, def_state, kaist_state):
        self.def_frames.append(def_state.copy())
        self.kaist_frames.append(kaist_state.copy())
        if len(self.def_frames) > self.num_frames:
            self.def_frames.pop(0)
            self.kaist_frames.pop(0)

    def _save_next_frame(self, next_state_def, next_state_kaist):
        self.def_frames2.append(next_state_def.copy())
        self.kaist_frames2.append(next_state_kaist.copy())
        if len(self.def_frames2) > self.num_frames:
            self.def_frames2.pop(0)
            self.kaist_frames2.pop(0)
    ...
    def convert_obs(self, o):
        return self.convert_def_obs(o), self.convert_kaist_obs(o)

    def convert_def_obs(self, o):
        li_vect=  []
    ...
    def _batch_train(self, training_step, step):
        ...
        s_batch_def = sample_batch[0]
        s_batch_kaist = sample_batch[1]
        a_batch = sample_batch[2]
        r_batch = sample_batch[3]
        s2_batch_def = sample_batch[4]
        s2_batch_kaist = sample_batch[5]
        ...
        def_input_size = self.observation_size * self.num_frames
        kaist_input_size = self.action_space.dim_topo * 6 * self.num_frames
        #print(input_size)
        input_t_def = np.reshape(s_batch_def, (self.batch_size, def_input_size))
        input_t_kaist = np.reshape(s_batch_kaist, (self.batch_size, kaist_input_size))
        input_t_1_def = np.reshape(s2_batch_def, (self.batch_size, def_input_size))
        input_t_1_kaist = np.reshape(s2_batch_kaist, (self.batch_size, kaist_input_size))
        ...
        # T Batch predict
        Q = self.policy_net.model.predict([input_t_def, input_t_kaist], batch_size = self.batch_size)
        ...
        # T+1 batch predict
        Q1 = self.policy_net.model.predict([input_t_1_def, input_t_1_kaist], batch_size=self.batch_size)
        Q2 = self.target_net.model.predict([input_t_1_def, input_t_1_kaist], batch_size=self.batch_size)
        ...
        # Batch train
        loss = self.policy_net.train_on_batch(input_t_def, input_t_kaist, Q, w_batch)
