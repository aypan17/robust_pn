...
class D3QN_Opponent(BaseOpponent):
    """
    This opponent will disconnect lines by learning Q-values of attacks among the attackable lines `lines_to_attack`.
    When an attack becomes possible, the time of the attack will be sampled uniformly
    in the next `attack_period` steps (see init).
    """

    def __init__(self, action_space, observation_space, lines_to_attack=[], attack_period=12*24, attack_duration=10,
    ...
    def tell_attack_continues(self, observation=None,
                              agent_action=None, env_action=None, budget=None):
        self._next_attack_time = None
    ...
    def attack(self, observation, agent_action=None, env_action=None, budget=None, previous_fails=False):
        ...
        # Epsilon variation
        r = np.random.rand(1)
        if r < self.epsilon and self.is_training:
    ...
    def _adaptive_epsilon_decay(self, step):
        ...
        ada_eps = self.initial_epsilon * -math.log10((step_off + 1) / (self.decay_epsilon + ada_div))
    ...
    def _save_hyperparameters(self, logpath, env, steps):
        ...
        hp = {
            ...
            "iter": steps,
            "e_start": self.initial_epsilon,
            "e_end": self.final_epsilon,
            "e_decay": self.decay_epsilon,
